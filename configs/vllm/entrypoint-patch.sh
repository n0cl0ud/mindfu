...
# Fixes JSONDecodeError when tool call arguments contain malformed JSON
...
if old in content:
    ...
    with open(path, 'w') as f:
        f.write(content)
    print('[entrypoint-patch] Patch applied successfully')
else:
    print('[entrypoint-patch] Pattern not found, skipping (may be different vLLM version)')
"
else
    echo "[entrypoint-patch] Patch already applied, skipping"
fi
