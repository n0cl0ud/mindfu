...
#
# PATCHED VERSION: Dynamic format detection for Devstral compatibility
# See: https://github.com/vllm-project/vllm/issues/23180
...
def _is_pre_v11_tokeniser(model_tokenizer: TokenizerLike) -> bool:
    """Original tokenizer-based detection (fallback)."""
    return not (
        isinstance(model_tokenizer, MistralTokenizer) and model_tokenizer.version >= 11
    )


def _detect_format_from_output(raw_tool_call: str) -> bool:
    """
    PATCH: Dynamically detect if output uses pre-v11 format based on actual content.

    - pre-v11: `[{"name": ..., "arguments": ...}]` (starts with '[')
    - v11+: `tool_name{args}` (starts with function name)

    Returns True if pre-v11 format is detected.
    """
    stripped = raw_tool_call.strip()
    if not stripped:
        return True  # Default to pre-v11 if empty

    # If starts with '[', it's pre-v11 JSON array format
    # If starts with anything else (function name), it's v11+ format
...
class MistralToolParser(ToolParser):
    ...
    PATCHED: Added dynamic format detection for Devstral compatibility.
    """
    ...
    def __init__(self, tokenizer: TokenizerLike):
        ...
        # PATCH: Track detected format (None = not yet detected)
        self._detected_is_pre_v11: bool | None = None

        # Use tokenizer-based detection as initial hint
        self._tokenizer_based_pre_v11 = _is_pre_v11_tokeniser(self.model_tokenizer)

        if self._tokenizer_based_pre_v11:
        ...
        self._is_pre_v11 = self._tokenizer_based_pre_v11
    ...
    def _init_pre_v11_parser(self):
        """PATCH: Initialize pre-v11 parser on demand when format is detected."""
        if self.parse_coro is None:
            self.parse_coro = ijson.parse_coro(
                self.update_stream_state_pre_v11_tokenizer()
            )
    ...
    def extract_tool_calls(
        ...
        PATCHED: Uses dynamic format detection based on actual output.
        """
        ...
        # PATCH: Dynamic format detection from actual output
        if raw_tool_calls:
            first_raw = raw_tool_calls[0]
            detected_pre_v11 = _detect_format_from_output(first_raw)

            # Cache detection and update state
            if self._detected_is_pre_v11 is None:
                self._detected_is_pre_v11 = detected_pre_v11
                self._is_pre_v11 = detected_pre_v11
                logger.info(f"PATCH: Detected tool call format: {'pre-v11' if detected_pre_v11 else 'v11+'}")

            is_pre_v11 = detected_pre_v11
        else:
            is_pre_v11 = self._is_pre_v11
    ...
    def extract_tool_calls_streaming(
        ...
        try:
            # PATCH: Dynamic format detection for streaming
            if self._detected_is_pre_v11 is None and self.bot_token in current_text:
                # Extract what comes after the first tool call token
                parts = current_text.split(self.bot_token, 1)
                if len(parts) > 1 and parts[1]:
                    raw_after_token = parts[1]
                    detected_pre_v11 = _detect_format_from_output(raw_after_token)
                    self._detected_is_pre_v11 = detected_pre_v11
                    self._is_pre_v11 = detected_pre_v11
                    logger.info(f"PATCH: Streaming detected tool call format: {'pre-v11' if detected_pre_v11 else 'v11+'}")

                    # Initialize pre-v11 parser if needed
                    if detected_pre_v11 and self.parse_coro is None:
                        self._init_pre_v11_parser()
